...
# We have two main steps of computation, each with its own parameter to be set by the user: 
#1. defining the tertiles and which genes (nodes) are abundant - user sets the abundance threshold; 
#2. computing which interactions (edges) are concordant - user sets the concordance threshold

#If a user changes the abundance threshold (regardless of changes in the concordant threshold) all computations need to be remade
#If a user changes the concordant threshold, but doesn't change the abundance network, only step #2 needs to be recomputed.

#In the future:
#- user input of genes of interest
#- user decides if only immunomodulators, all genes, edges with at least one immunomodulator,... are going to be used to build the network
