...
keep.node <- function(val,upbinfrac.threshold=0.75){val>=upbinfrac.threshold} ## helper function for thresholding upper bin ratio
...
addPerGroupIncludes <- function(df){
  df %>% group_by(Group) %>% nest() %>% ## split by Group, now have tibble with one row per Group; tibble named data; has ParticipantBarcode, Bin
    mutate(BinDistribution=map(data,unibin)) %>% ## add BinDistribution tibble with frequency of samples in each tertile bin
    mutate(UpBinRatio=map_dbl(BinDistribution,upbinfrac)) %>% ## add UpBinRatio with ratio of upper two bins to all (three) bins
    mutate(Include=map_lgl(UpBinRatio,keep.node)) %>%  ## creat logical for whether node shoud be kept (per group), based on keep.node
    select(Group,Include) %>% arrange(Group) ## simplify
 }

#V2 - output in file for use in iAtlas 
addPerGroupIncludes <- function(df){
  df %>% group_by(Group) %>% nest() %>% ## split by Group, now have tibble with one row per Group; tibble named data; has ParticipantBarcode, Bin
    mutate(BinDistribution=map(data,unibin)) %>% ## add BinDistribution tibble with frequency of samples in each tertile bin
    mutate(UpBinRatio=map_dbl(BinDistribution,upbinfrac)) %>% ## add UpBinRatio with ratio of upper two bins to all (three) bins
    select(Group,UpBinRatio) %>% arrange(Group) ## simplify
  #mutate(Include=map_lgl(UpBinRatio,keep.node)) %>%  ## creat logical for whether node shoud be kept (per group), based on keep.node
  #select(Group,Include) %>% arrange(Group) ## simplify
}
...
tabulate_pair <- function (df){
  df %>% group_by(Group) %>% nest() %>% mutate(t=map(data,table)) %>% select(Group,t)
}
...
getGroupRatioScores <- function(pt){
  ...
  pt %>% ungroup() %>% mutate(ratioScore=map(t,getRatioScore)) %>% 
...
##
filterRatioWithIncludes <- function (rscores,from_node,to_node,concordance_treshold=2.94){ # concordance_treshold=1.62
