...
ioresponse <- function(input, 
    ...
    output$survplot_op <- renderUI({
        ...
        var_choices <- colnames(panimmune_sigs[2:6])
        selectInput(
            ns("var1_surv"),
            "Variable",
            var_choices,
    ...
    feature_df <- reactive({
        ...
        survival_data <- sample %>%
            filter(Dataset %in% input$datasets & Treatment == "Pre") %>%
        ...
        merge(survival_data, var_data, by = "Sample_ID")
    })
...
#forest plot     
    output$forest <- renderPlotly({
        ...
        all_hr <- purrr::map(.x = input$datasets, data = feature_df, variable = input$var1_surv, .f= function(dataset, data, variable){
            ...
            survival::coxph(as.formula(paste0("survival::Surv(OS_time, OS) ~", variable)), data_cox)
        })
